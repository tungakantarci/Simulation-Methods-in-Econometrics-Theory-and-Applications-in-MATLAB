...
[H,~] = halton(N,dimensions,draws,'prime',prime);
...
figure
hold on
scatter(H(:,1),0.4*ones(N,1),'DisplayName',"Halton draws");
...
[H_standard_2D,Z_standard_2D] = halton(N,dimensions,draws, ...
    'prime',prime,'burn',250);

% 7.2. Scrambled Halton
[H_scramble_2D,Z_scramble_2D] = halton(N,dimensions,draws, ...
    'prime',prime,'scramble',1,'burn',250);

% 7.3. Leap Halton
[H_leap_2D,Z_leap_2D] = halton(N,dimensions,draws, ...
    'prime',prime,'leap',4,'burn',250);

% 7.4. Randomized Halton
[H_random_2D,Z_random_2D] = halton(N,dimensions,draws, ...
    'prime',prime,'random',1,'burn',250);
...
scatter(H_standard_2D(:,1),H_standard_2D(:,2), ...
    'DisplayName',"Standard Halton sequence")
ylabel("Dimension 2")
xlabel("Dimension 1")
legend('show')
...
scatter(H_scramble_2D(:,1),H_scramble_2D(:,2), ...
    'DisplayName',"Scrambled Halton sequence")
ylabel("Dimension 2")
xlabel("Dimension 1")
legend('show')
