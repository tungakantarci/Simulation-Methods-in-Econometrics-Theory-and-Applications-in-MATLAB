% Exercise – Understanding rejection sampling with Gaussian–oscillatory
...
% samples under different proposal distributions (uniform and normal), the
% exercise illustrates how the choice of proposal affects efficiency and
% accuracy.
...
for i = 1:length(x)
    ...
    if abs(x(i)) <= 3
        ...
        target_function(i) = exp(-0.5*x(i).^2).*oscillatory_component(i);
    else
...
uniform_points_combined = [uniform_points(:,1) uniform_points_scaled(:,1)];
...
% 12.2. Compute corresponding y-coordinates using the standard normal PDF
...
for k = 1:N_samples
    ...
    if y_coordinate <= target_function(index)
        % Accepted sample: point lies below target function (store in blue 
        % region)
        region_blue(k,1) = x_coordinate;
        region_blue(k,2) = y_coordinate;
    else
        % Rejected sample: point lies above target function (store in black 
        % region)
